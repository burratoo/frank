...
package body Atmel.AT91SAM7S.SPI is
   ...
   begin
      Serial_Peripheral_Interface.Setup_Chip_Select_Pin (For_Pin, Chip_Select_Settings);
   ...
   protected body Serial_Peripheral_Interface is
      ...
      is
         ...
         --  State of the Mode Register prior to the transmission

      begin
         --  Clear the Transfer_Completed flag prior to starting the transmission
         ...
         --  Set the SPI unit to communicate with the selected device

         MR.Peripheral_Chip_Select := With_Device;
         Mode_Register := MR;

         --  Configure the SPI unit to transmit a message if there is one
         ...
         --  Configure the SPI unit to receive a message if there is one

         if Recieve_Message /= Null_Address and then Recieve_Message_Length > 0 then
            Receive_Pointer_Register := Recieve_Message;
            Receive_Counter_Register := Recieve_Message_Length;
         end if;

         --  Enable either the End_Of_Transmit_Buffer or End_Of_Receive_Buffer interrupt
         --  depending on the longer message. This allows a task to block on the
         --  Wait_For_Transmission entry while the transmission occurs.
      ...
      begin
         --  If the transmission buffers are empty then the transmission is completed, so
         --  disable the interrupts and signal the transmission is completed.
