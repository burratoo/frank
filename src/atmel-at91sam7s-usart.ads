...
--  Driver for the ATMEL AT91SAM7S USART peripheral

--  ??? 2021: Could be improved so there are not seperate data paths for the two USART
--  peripherals.
...
package Atmel.AT91SAM7S.USART is
   ...
   --  Initialise the USART peripheral with ID Interface_Id with the provided
   --  configuration parameters.

   procedure Reset_Interface (Interface_Id : in USART_Id);
   --  Reset the USART peripheral with ID Interface_Id
   ...
   --  Exchange messages with USART device on the desired USART interface. To accommodate
   --  different message types the procedure is passed the messages' address and length.
   --  The call is synchronous and will block the task while it waits for the return
   --  message.

   function Interface_Is_Ready (Interface_Id : USART_Id) return Boolean;
   --  Returns true if the clock for the USART interface Interface_Id is stable and the
   --  interface is ready to send or receive data.

   function Receive_Bytes_Remaining (On_Interface : USART_Id) return Natural;
   --  Number of bytes remaining to be received on USART interface Interface_Id for the
   --  current operation.
...
--     end USART_Interface_0;
   ...
   --  The USART_Interface_1 protected object handles the communication with the
   --  USART_Interface_1. Using the protected object ensures only one task access USART 1
   --  at a time. A protected entry, Wait_For_Transmission, allows a task to block while
   --  the message exchange occurs.

   --  ??? Could turn this into a generic package so the same code can be used for both
   --  USART interfaces.

   protected USART_Interface_1 with Interrupt_Priority => Max_Interrupt_Priority is

      procedure Initialise_Interface
        (USART_Settings   : in Mode_Type;
         Receiver_Timeout : in Unsigned_16;
         Baud_Rate        : in Baud_Rate_Generator_Type);
      --  Initialise the USART peripheral interface with the given settings
      ...
      procedure Exchange_Data
        (Send_Message           : in Address;
         ...
         Recieve_Message        : in Address;
         Recieve_Message_Length : in Unsigned_16);
      --  Exchange data asynchronously with the USART 1 using the provided message
      --  buffers. Procedure will return once the USART 1 is configured to transmit and
      --  receive the messages from the provided message buffers.

      --  ??? 2021: this should be an entry to prevent another task calling Exchange_Data
      --  while an exchange is occurring.

      entry Wait_For_Transmission;
      --  Wait for the message exchange to occur

      procedure Reset_Interface;
      --  Reset the USART 1 interface

   private

      procedure Interface_Handler with Attach_Handler => US1_Interrupt;
      --  Handle the interrupts from the USART 1 interrupts

      Transmit_Buffer        : Address;
      Transmit_Buffer_Length : Natural;
      --  Address of the message to transmit and its length

      Recieve_Buffer         : Address;
      Recieve_Buffer_Length  : Natural;
      --  Address and length of the message buffer to store the received message

      Transfer_Completed : Boolean := False;
      --  Indicates when whether an exchange has completed
