...
--  Driver for the ATMEL AT91SAM7S SPI peripheral
...
package Atmel.AT91SAM7S.SPI is
   ...
   type Chip_Select_Pins is array (Peripheral_Chip_Select_Pin) of Peripheral_Pin;
   ...
   --  Exchange messages with SPI slave device. To accommodate different message types the
   --  procedure is passed the messages' address and length. The call is synchronous and
   --  will block the task while it waits for the return message.
   --
   --  ??? 2021: not sure what motivated me to pass the message Address and length here.
   --  It would have been better to use an array of bytes.

   procedure Initialise_Interface
     (SPI_Settings                : in Mode_Type;
      Chip_Select_Pin_Assignments : in Chip_Select_Pins);
   --  Initialise the SPI peripheral interface with the given settings

   procedure Setup_Chip_Select_Pin
     (For_Pin              : in Peripheral_Chip_Select_Pin;
      Chip_Select_Settings : in Chip_Select_Type);
   --  Configure the settings for the specified SPI chip select
...
private
   ...
   --  The Serial_Peripheral_Interface protected object handles the communication with the
   --  SPI device. Using the protected object ensures only one task access the SPI bus at
   --  a time. A protected entry, Wait_For_Transmission, allows a task to block while the
   ...
   protected Serial_Peripheral_Interface is
      ...
      procedure Exchange_Data
        (With_Device            : in Peripheral_Chip_Select_Id;
         ...
         Recieve_Message        : in Address;
         Recieve_Message_Length : in Unsigned_16);
      --  Exchange data asynchronously with the select SPI slave device using the provided
      --  message buffers. Procedure will return once the SPI unit is configured to
      --  transmit/receive.
      ...
      procedure Setup_Chip_Select_Pin
        (For_Pin              : in Peripheral_Chip_Select_Pin;
         Chip_Select_Settings : in Chip_Select_Type);
      --  Configure the per Chip Select Pin settings
   ...
   private
      ...
      procedure Interface_Handler with Attach_Handler => SPI_Interrupt;
      --  Handle the transmission buffers empty interrupt
