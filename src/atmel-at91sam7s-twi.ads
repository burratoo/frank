...
--  Driver for the ATMEL AT91SAM7S Two Wire Interface (TWI) peripheral
...
package Atmel.AT91SAM7S.TWI is
   ...
   --  Send the given message over the TWI bus to the specified device. Internal_Address
   --  is the address on the device to which the message will be sent to, with
   --  Address_Size the size in bytes. The call is synchronous and will block the task
   --  while it waits for the return message.
   ...
   procedure Receive_Message
     (From             : in TWI_Device_Address;
      ...
      Message          : in Address;
      Message_Length   : in Natural);
   --  Receive a message from the specified device to the Message buffer. Internal_Address
   --  is the address on the device to which the message will be sent to, with
   --  Address_Size the size in bytes. The call is synchronous and will block the task
   --  while it waits for the return message.
...
private
   ...
   --  The Two_Wire_Interface protected object handles the communication with the TWI
   --  device. Using the protected object ensures only one task access the TWI bus at
   ...
   is
      ...
      procedure Initialise_Interface
        (Clock_Divider      : Clock_Divider_Type;
         Clock_Low_Divider  : Unsigned_8;
         Clock_High_Divider : Unsigned_8);
      --  Initialise the TWI peripheral interface with the given settings
      ...
      --  Depending on Direction, send or receive the given message over the TWI bus to the
      --  specified device. Internal_Address is the address on the device to which the
      --  message will be sent to, with Address_Size the size in bytes. The call is
      --  asynchronous and will return after the transmission has started.

      entry Wait_For_Transmission;
      --  Wait for the asynchronous message transmission to occur

   private

      procedure Interface_Handler with Attach_Handler => TWI_Interrupt;
      --  Handle interrupts from the TWI unit to facilitate loading of data to and from
      --  the message and TWI buffers.
